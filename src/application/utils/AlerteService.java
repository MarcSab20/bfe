...
import application.models.Alerte;
...
public class AlerteService {
    ...
    public List<Alerte> getAlertesActives() {
        ...
        try {
            ...
            String sql = "SELECT a.*, s.nom AS nom_stagiaire, s.prenom, st.type AS type_stage, " +
                        "st.date_fin, e.nom AS nom_ecole, e.pays " +
                        "FROM alertes a " +
    ...
    public List<Alerte> getAlertesUrgentes(int joursLimite) {
        ...
        try {
            ...
            String sql = "SELECT st.id AS stage_id, s.id AS stagiaire_id, " +
                        ...
                        "st.type, st.date_fin, e.nom AS nom_ecole, e.pays " +
                        "FROM stages st " +
                        "JOIN stagiaires s ON st.stagiaire_id = s.id " +
                        "JOIN ecoles e ON st.ecole_id = e.id " +
            ...
            while (rs.next()) {
                ...
                alerte.setTypeStagiaire(rs.getString("type"));
                alerte.setNomEcole(rs.getString("nom_ecole"));
                alerte.setPays(rs.getString("pays"));
                alerte.setDateFinFormation(rs.getObject("date_fin", LocalDate.class));
    ...
    /**
     * Générer automatiquement les alertes pour les stages se terminant bientôt
     */
    public int genererAlertesAutomatiques(int joursAvance) {
        ...
        try {
            ...
            // Trouver les stages qui nécessitent une alerte
            String sqlSelect = "SELECT st.id AS stage_id, st.stagiaire_id " +
                             "FROM stages st " +
            ...
            stmtSelect.setInt(1, joursAvance);
            
            ResultSet rs = stmtSelect.executeQuery();
            
            String sqlInsert = "INSERT INTO alertes (stage_id, stagiaire_id, lue, date_creation) " +
                             "VALUES (?, ?, false, NOW())";
            PreparedStatement stmtInsert = conn.prepareStatement(sqlInsert);
            
            while (rs.next()) {
                stmtInsert.setLong(1, rs.getLong("stage_id"));
                stmtInsert.setLong(2, rs.getLong("stagiaire_id"));
                stmtInsert.addBatch();
                count++;
            }
